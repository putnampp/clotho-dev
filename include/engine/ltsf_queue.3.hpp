...
public:
    ...
    //    _object( size_t idx, object_ptr o = NULL, size_t q = UNKNOWN_INDEX, size_t n = UNKNOWN_INDEX, size_t p = UNKNOWN_INDEX) : 
    //        node_idx(idx), 
    //        object(o), 
    //        queue(q), 
    //        next(n), 
...
//        _node & onode = m_nodes[o_idx];
//        onode._data._object.object = o;
...
//        if( m_root != UNKNOWN_INDEX ) {
        if( res != NULL ) {
            ...
            if( rnode._data._ltsf.head == UNKNOWN_INDEX ) {
...
//        assert( t != (vtime_t) -1 );
//        assert( pool_idx != UNKNOWN_INDEX && pool_idx < m_nodes.size() );
//        _node & obj = m_objects[pool_idx];
//        updateObject( obj, t );
//    }
...
//    void updateObject( _node & obj, vtime_t t ) {
...
protected:
    ...
    size_t getObjectNode( object_ptr o ) {
        size_t n = getNode();
        m_nodes[n]._data._object.object = o;
        m_nodes[n]._data._object.queue = UNKNOWN_INDEX;
        return n;
    }

    size_t getLTSFNode( vtime_t t ) {
        size_t q = getNode();
        m_nodes[q]._data._ltsf.timestamp = t;
        m_nodes[q]._data._ltsf.head = UNKNOWN_INDEX;
        return q;
    }
...
//    void insert_object_to_ltsf_node( _node & o, size_t q_idx ) {
    ...
    object_ptr m_head;
    size_t m_available_node;
//    size_t m_unset_object;      // _object index
